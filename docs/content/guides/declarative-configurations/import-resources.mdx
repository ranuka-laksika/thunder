...
Thunder provides the `POST /import` API to import declarative YAML content into runtime stores.
...
When import runs with `dryRun=false`, Thunder persists resource changes in runtime stores.

When `POST /import/delete` succeeds, Thunder removes the matching declarative YAML file from `repository/resources`.
